params ["_cargoBox"];
if (!isNull _cargoBox) exitWith {
	clearItemCargo _cargoBox;
	...
	for "_i" from 0 to _len do {
		_cargoBox addItemCargo [_zzjj_noob_loadout_a select _i, _zzjj_noob_loadout_b select _i];
	};
	_cargoBox addItemCargoGlobal ["ToolKit", 1];//同步所有
};