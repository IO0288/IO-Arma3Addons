...
with uiNamespace do {
	SLTScriptDisplayName = "Team Name Tags";
};
			...
			TeamNameTagEvent = addMissionEventHandler ["Draw3D", {
				private _vehicleList = [];
				{
					if (((side group _x) isEqualTo (side group player)) && (_x != player)) then {
						...
						_driver = if (driver vehicle _x isEqualTo objNull) then {
							effectiveCommander vehicle _x
						} else {
							driver vehicle _x
						};
						_textSize = 0.0325;
						...
						if (vehicle _x == cursorTarget) then {
							_alpha = 1;
						};
						...
						{
							case "COLONEL": {
								"\a3\ui_f\data\GUI\cfg\Ranks\colonel_pr.paa"
							};
							case "MAJOR": {
								"\a3\ui_f\data\GUI\cfg\Ranks\major_pr.paa"
							};
							case "CAPTAIN": {
								"\a3\ui_f\data\GUI\cfg\Ranks\captain_pr.paa"
							};
							case "LIEUTENANT": {
								"\a3\ui_f\data\GUI\cfg\Ranks\lieutenant_pr.paa"
							};
							case "SERGEANT": {
								"\a3\ui_f\data\GUI\cfg\Ranks\sergeant_pr.paa"
							};
							case "CORPORAL": {
								"\a3\ui_f\data\GUI\cfg\Ranks\corporal_pr.paa"
							};
							case "PRIVATE": {
						...
						if ((rank _x) isEqualTo "COLONEL") then {
							_imageSize = [0.75, 0.75];
							_color = [1, 1, 1, 1];
						};
						...
						if (vehicle _x != _x) then {
							if !((vehicle _x) in _vehicleList) then
							{
								...
								if ((_distance > TNTMaxDistanceUnitMarkerText3D) && (vehicle _x != cursorTarget)) then {
									_text = "";
								};
				...
				} forEach (if (count allPlayers isEqualTo 1) then {
					allUnits+allDeadMen
				} else {
					allPlayers
				});
