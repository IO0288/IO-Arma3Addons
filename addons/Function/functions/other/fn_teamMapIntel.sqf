/*
	 *IO_Batch_teamMapIntel.sqf
	 *Team Map Intel
	 *地图显示团队情报
	 *[] execVM "IO_Batch_teamMapIntel.sqf";
 */
...
with uiNamespace do {
	SLTScriptDisplayName = "Team Map Intel";
};
		...
		[] spawn {
			...
			RKAG_fnc_getGroupNATOMarkerType = {
				...
				_support = 0;
				{
					if (!canStand vehicle _x && alive vehicle _x && !(vehicle _x in _vehlist)) then {
						...
						if (_canReammo) then {
							_support_reammo = _support_reammo + 1
						};
						if (_canRefuel) then {
							_support_refuel = _support_refuel + 1
						};
						if (_canRepair) then {
							_support_repair = _support_repair + 1
						};
					};
				...
				if ((_support_reammo + _support_refuel) >= 1) then {
					_type = "support"
				};
				if (_support_repair >= 1) then {
					_type = "maint"
				};
				if (_support_medic >= 1) then {
					_type = "med"
				};
				if ((_support_medic + _support_reammo + _support_refuel + _support_repair) > 1) then {
					_type = "support"
				};
				if (_boats >= 1) then {
					_type = "naval"
			...
			RKAG_fnc_spawnRevealGroup = {
				...
				while { !(_group isEqualTo grpNull) && (_group in RKAGRevealedGroups) } do {
					...
					private _i = units _group findIf {
						alive _x
					};
					if (_i < 0) then {
				...
				private _j = units _group findIf {
					alive _x
				};
				if (!(_group isEqualTo grpNull) && (_j > -1) && (side _group != side player)) then {
			...
			RKAG_fnc_showMapTooltip = {
				...
				with uiNamespace do
				{
					...
					if !(_footText isEqualTo "") then
					{
						...
						RKAGMapInfantryTooltipBody ctrlSetStructuredText parseText ("<t size='0.1'>&#160;
						</t><br/>"+_footText);
						RKAGMapInfantryTooltipBody ctrlCommit 0;
					} else {
						RKAGMapInfantryTooltipHeader ctrlShow false;
					...
					if !(_vehicleText isEqualTo "") then
					{
						...
						RKAGMapVehicleTooltipBody ctrlSetStructuredText parseText ("<t size='0.5'>&#160;
						</t><br/>"+_vehicleText);
						RKAGMapVehicleTooltipBody ctrlCommit 0;
				...
				if (alive player) then {
					{
						...
						if (_i > -1 || (side _group isEqualTo side player && (count units _group <= 1) && !_inVehicle) || side _group isEqualTo civilian) then {
							continue
						};
