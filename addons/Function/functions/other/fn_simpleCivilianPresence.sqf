...
with uiNamespace do {
	SLTScriptDisplayName = "Simple Civilian Presence";
};
...
SLT_fnc_enableScript = {
	{
		...
		CIVPOP_fnc_startSystem = {
				...
				while { isPopulateWorldAllowed } do {
					{
						...
						if !(isNil "_isCivPopAgent") then
						{
							...
							if (isNil "_t") then {
								_agent setVariable ["deathTime", _deafultDelay];
							} else {
			...
			CIVPOP_fnc_deleteAllAgents = {
				{
					...
					if !(isNil "_isCivPopAgent") then
					{
						...
						if !(agent _x isEqualTo objNull) then {
			...
			CIVPOP_fnc_checkForNearbyBlacklistedObjects = {
				...
				_isValid = true;
				if (_road in allInvalidRoadCache) exitWith {
					false
				};
				...
				_terobjs = (_terobjs + (_road nearObjects ["Land_LandMark_F", _minDistanceBase]));
				{
					...
					if (!_isValid) exitWith {
						allInvalidRoadCache pushBack _road;
					};
			...
			CIVPOP_fnc_getAllPlayers = {
				...
				_return = [];
				_return = if (_includeZeus || (count allPlayers isEqualTo 1)) then {
					allPlayers
				} else {
			...
			CIVPOP_fnc_getVelocityAdjustedPosition = {
				...
				if (_vectorLength > _maxVectorLength) then {
					_vectorLength = _maxVectorLength;
				};
			...
			CIVPOP_fnc_updateValidSpawns = {
				...
				if (_debug) then {
					...
					{
						_mkr = createMarker["marker_"+(str count allMapMarkers), getPos _x];
			...
			CIVPOP_fnc_moveToNearbyRandomRoad = {
				...
				{
					if ((_unit distance _x) > _minDistance) then {
						_v pushBack _x;
					};
				} forEach _roads;
				...
				if (_rotate) then {
					vehicle _unit setDir ((getDir _unit)+(_unit getRelDir _dest))
				};
				...
				[_maxUnitCount, _maxRoads, _minTravenDistance, _maxTravelDistance, _deleteDistance, _updateRate, _debug] spawn {
					...
					while { isPedestriansAllowed && isPopulateWorldAllowed } do {
						...
						if !(isNil "_vs") then
						{
							...
							if (_modifiedUnitCount > _maxUnitCount) then {
								_modifiedUnitCount = _maxUnitCount
							};
						};
