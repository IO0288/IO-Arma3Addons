/*
	 *IO_Batch_simpleSpotSystem.sqf
	 *Simple Spot System
	 *简单标点系统, 使用T标记敌军
	 *[] execVM "IO_Batch_simpleSpotSystem.sqf";
 */
...
with uiNamespace do {
	SLTScriptDisplayName = "Simple Spot System";
};
...
SLT_fnc_enableScript = {
	[[], {
		...
		waitUntil {
			sleep 0.1;
			!((findDisplay 12 displayCtrl 51) isEqualTo controlNull)
		};
		...
		SPOT_fnc_spawnPlaySpotVoiceLine = {
			...
			{
				case "rus": {
					format ["A3\dubbing_radio_f_enoch\data\%1\%2\%3\010_Vehicles\%4", _language, _speakerType, "NORMALContact", "veh_unknown_p.ogg"]
				};
				case "pol": {
					format ["A3\dubbing_radio_f_enoch\data\%1\%2\%3\010_Vehicles\%4", _language, _speakerType, "NORMALContact", "veh_unknown_p.ogg"]
				};
				case "fre": {
					format ["A3\dubbing_radio_f_exp\data\%1\%2\RadioProtocol%3\%4\010_Vehicles\%5", _language, _speakerType, _language select [0, 3], "NORMALContact", "veh_unknown_p.ogg"]
				};
				case "engfre": {
					format ["A3\dubbing_radio_f_exp\data\%1\%2\RadioProtocol%3\%4\010_Vehicles\%5", _language, _speakerType, _language select [0, 3], "NORMALContact", "veh_unknown_p.ogg"]
				};
				...
				default {
					format ["A3\dubbing_radio_f\data\%1\%2\RadioProtocol%3\%4\010_Vehicles\%5", _language, _speakerType, _language select [0, 3], "NORMALContact", "veh_unknown_p.ogg"]
				};
			};
			...
			waitUntil {
				soundParams _soundID isEqualTo []
			};

			private _soundFilePath = switch (_language) do
			{
				case "rus": {
					format ["A3\dubbing_radio_f_enoch\data\%1\%2\%3\DirectionCompass1\%4", _language, _speakerType, "CombatEngage", _voiceFileName]
				};
				case "pol": {
					format ["A3\dubbing_radio_f_enoch\data\%1\%2\%3\DirectionCompass1\%4", _language, _speakerType, "CombatEngage", _voiceFileName]
				};
				case "fre": {
					format ["A3\dubbing_radio_f_exp\data\%1\%2\RadioProtocol%3\%4\DirectionCompass1\%5", _language, _speakerType, _language select [0, 3], "CombatEngage", _voiceFileName]
				};
				case "engfre": {
					format ["A3\dubbing_radio_f_exp\data\%1\%2\RadioProtocol%3\%4\DirectionCompass1\%5", _language, _speakerType, _language select [0, 3], "CombatEngage", _voiceFileName]
				};
				...
				default {
					format ["A3\dubbing_radio_f\data\%1\%2\RadioProtocol%3\%4\DirectionCompass1\%5", _language, _speakerType, _language select [0, 3], "CombatEngage", _voiceFileName]
				};
			};
		...
		SPOT_fnc_clientSpotTarget = {
			...
			if (!(_target in AllSpottedTargets) && (_target isKindOf "AllVehicles") && (alive _target) && !(_target in allPlayers) && !(driver _target in allPlayers)) then {
				...
				[AllSpottedTargets, {
					AllSpottedTargets = _this;
				}] call SPOT_fnc_RE_Server;
				playSound "hint3";

				if !(cameraView isEqualTo "GUNNER") then {
					[player, "gestureGo"] remoteExec ["playActionNow", 0];
				};
				if (!SpotIsPlayingVoice) then {
					[] spawn SPOT_fnc_spawnPlaySpotVoiceLine;
				};
			};
		};

		SPOT_fnc_getCursorTarget = {
			if (cursorTarget isEqualTo objNull) then {
				cursorObject
			} else {
				cursorTarget
			}
		...
		SpotKeyDownEvent = findDisplay 46 displayAddEventHandler ["KeyDown", {
			...
			if (_pressed && isSpotSystemAllowed) then {
				[call SPOT_fnc_getCursorTarget] call SPOT_fnc_clientSpotTarget;
			};
		}];
		...
		SpotMissionDrawEvent = addMissionEventHandler ["Draw3D", {
			{
				...
				if (vehicle _x isEqualTo cursorObject) then {
					_alpha = 1;
				} else {
					_text=""
				};
				if (_distance >= (getObjectViewDistance select 0)) then {
					_alpha = 0;
				};
		...
		SpotMapDrawEvent = (findDisplay 12 displayCtrl 51) ctrlAddEventHandler ["Draw", {
			_vehicleList = [];
			{
				...
				if (_x isKindOf "Man") then {
					...
					if (_scale > SpotMinMapZoomUnitMarker) then {
